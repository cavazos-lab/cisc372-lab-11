...
__global__ void matrixAdd(double *A, double *B, double *C, int N)
{
    ...
    // blockDim.y = 1, threadIdx.y will always = 0 
    // Note: blockDim.y == THREAD_DIMY (defined in host code)
    i = blockIdx.y;
...
int main(int argc, char*argv[])
{
    ...
    THREAD_DIMX = 256;
    THREAD_DIMY = 1;    /* fix THREAD_DIMY = 1  */

    /* The following five lines can take in parameters from the command line 
        e.g. ./matrixadd 4 128  */
    if (argc == 3)
    {
    ...
    BLOCK_DIMX = N/THREAD_DIMX;
    BLOCK_DIMY = N/THREAD_DIMY;
